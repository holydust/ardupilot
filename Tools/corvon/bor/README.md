...
### Gate 2 放电阈值/驻留标定 —— 2026-08-20,G2 首片

当初留这条,是因为没人证明过 `0x1FFFC000` 的读取不会也走加载副本。与其去做一次
受控断电,不如直接验读取路径:解锁 OPTCR 后**只改寄存器里的 BOR_LEV、不置 OPTSTRT**
(因此零 flash 写入),再重读同一地址:

```
OPTCR       0x0FFFAAED → 0x0FFFAAE0    加载副本已改成 Level 3
0x1FFFC000  0x5500AAFF → 0x5500AAFF    存储纹丝不动
```

随后真正写入 option byte,同一地址变成 `0x550CAAF3`(value 0xAAF3 / complement 0x550C)。

**两个方向都成立 → 该地址读的是 flash 存储,不是加载副本。** `bor_check.cfg` 的前提
有了实测背书;`bor_verify.cfg` 的受控断电不再是必需的伴随项,保留作工装定型时的旁证。
