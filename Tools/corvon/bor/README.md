...
## 🆕 2026-08-26:BOR 现在由固件自己设,本目录降级为后备与工程路径

**前提变了。** 本文档原先建立在"选项字节只能通过 SWD 写入"之上 —— 那句话不再成立。

`AP_Periph` 的 `CorvonGPS::bor_init()`(固件 `0b4eb6e1ed` 起)在 `init()` 最前面读 `OPTCR`,
**只在 `BOR_LEV` 不对时**写一次 Level 3,然后回读确认,并把结果打进串口 `ver`:

```
bor: ok level3                     <- 本来就对,没有写
bor: ok level3 (set this boot)     <- 检测到不对,已写好
bor: FAIL <原因> lev N rdp 0xXX    <- 失败
```

**放行判据 = `ver` 里出现 `bor: ok` 前缀。** Gate 1 与 Gate 2 都从"探针 + 万用表 + 受控断电"
变成"grep 一行"。

### 实测依据(G1,APM32F405,2026-08-26)

| 做了什么 | 结果 |
|---|---|
| 应用侧写 Level 3 → Level 2 | `OPTCR 0x0FFFAAE1 → 0x0FFFAAE5`,存储字 `0x550CAAF3 → 0x5508AAF7`,补码自洽 |
| **`V` 焊盘实测降到 0.01V 的深度断电** | **仍是 Level 2** —— 写入真的进了 NVM,不是 controller buffer 的假象 |
| 应用侧写回 Level 3 | 逐位回到基线,再次深度断电后仍保持,`bor_verify.cfg` 报 `GATE2 PASS` |
| **用 SWD 把 BOR 设成出厂默认(关闭),再普通重启** | 固件检测到不对、自行写入,`ver` 报 `bor: ok level3 (set this boot)`;SWD 独立复核 `0x550CAAF3` / `OPTCR 0x0FFFAAE1` |

### 两条实测出来的、会误导判据的性质

🔴 **`BSY` 不是可用的完成信号。** 本次全部写入在第一次轮询之前就已结束(`spins 0`)。
任何"等 `BSY` 落下即视为完成"的判据在这块硅片上是**空转的**。

🔴 **`0x1FFFC000` 在编程期间会经过补码不自洽的中间状态,而且可复现。**
`OPTSTRT` 之后立刻读到 `0x5500AAF3`(值与补码不匹配),约一秒后同一地址读到 `0x5500AAFF`。
**这说明"撕裂的 option block"在本硅片上不是假想** —— 掉电正好落在这个窗口里就会留下一个坏字。
因此完成判据必须是**回读到自洽为止**(固件里是 20 次 × 5ms),不是等忙。

### 这对"能不能退役 Gate 2"意味着什么

**部分推进,没有完全解决。**

- ✅ 固件在**每次自然上电**时读的是复位后从 flash 重装的 `OPTCR` —— 那**就是 Gate 2 的语义**,
  而且每台、每次开机免费做一遍,比 `bor_check` 的即时读更强
- ✅ 第一次拿到了**判据成功拒绝非合规状态**的实例(BOR 关闭 → 被检出并纠正)
- 🔴 **仍然没有"NVM commit 静默失败"的样本。** known-bad 标定这一条没有解除,
  只是把它从"补脚本能不能解决"变成"要不要为它专门制造失败样本"

### 本目录现在的定位

openocd 这套仍然保留,用途变成三条:**① 产线首烧时顺手设(冗余,不再是唯一途径);
② 独立复核固件的自设结果(不同的读取路径);③ 工程排障。**
`bor_verify.cfg` 仍不得退役,理由见上。
